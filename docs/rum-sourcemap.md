...
RUM 默认关闭。先在 Guance 创建 Web RUM 应用，取得非敏感的 application ID，再设置：
...
产物位于 `dist/guance-observability-demo-rum-sourcemap.zip`，`dist/` 不进入 Git。上传 SourceMap 时，service、env、version 和线上资源路径必须与 RUM 事件一致；本 Demo 默认 service 为 `mall-h5`，version 来自 `DD_VERSION` 或镜像 tag。
